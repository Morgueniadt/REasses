<nav x-data="{ open: false }" class="bg-black dark:bg-gray-900 border-b border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('note.index')" :active="request()->routeIs('note.index')" class="text-white hover:text-gray-300">
                        {{ __('View All Notes') }}
                    </x-nav-link>
    ...
    <form method="POST" action="{{ route('logout') }}">
        @csrf
        <x-dropdown-link :href="route('logout')" class="text-white hover:text-gray-300"
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('note.index')" :active="request()->routeIs('note.index')" class="text-white hover:text-gray-300">
                {{ __('View All Notes') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('note.create')" :active="request()->routeIs('note.create')" class="text-white hover:text-gray-300">
                {{ __('Create New Note') }}
            </x-responsive-nav-link>
        </div>
